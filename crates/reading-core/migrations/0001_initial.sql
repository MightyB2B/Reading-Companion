...
--
-- Ported from the original SQLite schema. Four differences worth naming:
-- integer flags are real BOOLEANs, timestamps are TIMESTAMPTZ rather than
-- strings, `books.last_page_id` is added by ALTER at the end because Postgres
-- will not accept a reference to a table that does not exist yet, and every
-- integer is BIGINT.
--
-- That last one is not fussiness. SQLite has one integer type, so the Rust
-- side settled on i64 throughout; Postgres would map INTEGER to i32 and the
-- mismatch surfaces as a decode error at runtime, on whichever query happens
-- to run first. BIGINT everywhere makes the two sides agree by construction.
